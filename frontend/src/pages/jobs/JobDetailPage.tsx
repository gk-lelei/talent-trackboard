...
import { useJobs, JobPosting } from "@/context/JobsContext";

const JobDetailPage = () => {
  const { jobId } = useParams<{ jobId: string }>();
  const navigate = useNavigate();
  const [showPaymentModal, setShowPaymentModal] = useState(false);
  const { canApplyForJob, jobApplicationsCount, jobApplicationsLimit, isPremium } = useSubscription();
  const { jobs } = useJobs();
  ...
  const similarJobs = jobs.filter(j => 
    j.id !== parseInt(jobId!) && 
    job && j.department === job.department && 
    j.status === "active"
  ).slice(0, 2);
    ...
    return (
      <div className="container mx-auto py-8">
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="bg-white rounded-lg shadow-sm animate-pulse border border-gray-100 p-8 h-64"></div>
        </div>
      </div>
    );
  }
